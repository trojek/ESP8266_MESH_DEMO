...
void mesh_none_proto_parser(void *arg, uint8_t *pdata, uint16_t len)
{
    uint16_t op_idx = 1;
...
void mesh_topo_test()
{
    ...
    option = (struct mesh_header_option_format *)espconn_mesh_create_option(M_O_TOPO_REQ, dst, sizeof(dst));
    if (!option) {
...
void mesh_topo_test_start()
{
    static os_timer_t tst_timer;
    os_timer_disarm(&tst_timer);
    os_timer_setfn(&tst_timer, (os_timer_func_t *)mesh_topo_test, NULL);
    os_timer_arm(&tst_timer, 8000, true);
}
